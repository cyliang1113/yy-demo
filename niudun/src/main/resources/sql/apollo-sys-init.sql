...
CREATE TABLE `sys_user` (
  ...
  `emp_no` varchar(100) NOT NULL DEFAULT '' COMMENT '工号',
  ...
  INDEX `idx_sys_user_user_code` (`user_code`) USING BTREE ,
...
CREATE TABLE `sys_resource` (
  ...
  INDEX `idx_sys_resource_resource_code` (`resource_code`) USING BTREE ,
...
(
  ...
  COMMENT '主键',
  dict_type   VARCHAR(190)           NOT NULL default ''
  COMMENT '字典类型',
  dict_type_name        VARCHAR(190)           NOT NULL default ''
  COMMENT '字典类型名称',
  dict_key VARCHAR(190)    NOT NULL default ''
  COMMENT '字典-key',
  ...
  COMMENT '修改人ID',
  PRIMARY KEY(`id`),
  INDEX `idx_sys_dict_dict_type_key` (`dict_type`,`dict_key`) USING BTREE ,
